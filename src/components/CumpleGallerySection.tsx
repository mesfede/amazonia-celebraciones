...
export const CumpleGallerySection: React.FC = () => {
  ...
  return (
    <section id="galeria" className="py-16 sm:py-24 bg-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="text-center max-w-2xl mx-auto mb-14">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-teal-50 text-teal-700 font-kids text-sm font-semibold mb-3 border border-teal-200 shadow-xs">
          ...
          <p className="text-slate-600 text-base sm:text-lg mt-2">
            Risas, juegos y pura diversión en cada festejo. Tocá cualquier foto para abrir la galería completa.
          </p>
        </div>

        {/* Prominent Photo Cards Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 sm:gap-6 lg:gap-8 pt-4 pb-8 max-w-7xl mx-auto">
          {CUMPLE_PHOTOS.map((item, index) => (
            <div
              key={index}
              onClick={() => setSelectedPhoto(index)}
              className={`relative cursor-pointer transition-all duration-300 transform ${item.rotate} hover:scale-105 hover:z-20 group`}
            >
              {/* Playful Tape / Sticker Decoration */}
              <div
                className={`absolute -top-3.5 left-1/2 -translate-x-1/2 w-24 h-7 ${item.tapeColor} backdrop-blur-xs rounded-sm -rotate-2 shadow-sm z-20 border-y border-white/40`}
              />

              {/* Photo Frame (Polaroid / Sticker Style) */}
              <div className="bg-white p-3.5 pb-5 rounded-3xl shadow-xl hover:shadow-2xl border-2 border-slate-100 hover:border-teal-300 transition-all duration-300">
                <div className="relative aspect-[3/4] sm:aspect-[4/5] overflow-hidden rounded-2xl bg-slate-900">
                  <img
                    src={item.src}
                    alt={item.alt}
                  ...
                  <div className="absolute inset-0 bg-black/25 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                    <div className="w-12 h-12 rounded-full bg-white/95 text-teal-700 flex items-center justify-center shadow-xl transform scale-75 group-hover:scale-100 transition-transform">
        ...
        <div className="mt-8 text-center">
          <a
            ...
            className="inline-flex items-center gap-3 px-8 py-4 rounded-full bg-teal-600 hover:bg-teal-700 text-white font-kids text-lg font-bold shadow-lg shadow-teal-600/25 hover:scale-105 active:scale-95 transition-all"
          >
            <MessageCircle className="w-5 h-5 text-amber-300" />
            ¡Quiero festejar en Amazonia!
          </a>
        </div>
      ...
      {selectedPhoto !== null && (
        <div 
          className="fixed inset-0 z-50 bg-slate-950/95 backdrop-blur-xl flex flex-col items-center justify-between p-4 sm:p-6 select-none animate-in fade-in duration-200"
        ...
        >
          ...
          <div className="relative z-20 w-full max-w-5xl flex items-center justify-between pt-2 pb-2">
            <div className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/15 backdrop-blur-md border border-white/20 text-white font-kids text-sm font-bold shadow-lg">
            ...
            <button
              ...
              className="w-11 h-11 rounded-full bg-white/15 hover:bg-white/30 text-white flex items-center justify-center backdrop-blur-md transition-all z-50 cursor-pointer border border-white/25 hover:scale-105 active:scale-95"
          ...
          <div 
            className="relative z-20 w-full max-w-5xl flex-1 flex items-center justify-center px-2 sm:px-12 my-auto"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Left Nav Arrow */}
            <button
              onClick={handlePrev}
              className="absolute left-1 sm:left-2 top-1/2 -translate-y-1/2 w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-amber-400 hover:bg-amber-300 text-slate-950 flex items-center justify-center shadow-2xl transition-all transform hover:scale-110 active:scale-95 cursor-pointer z-30 border-2 border-white/60"
            ...
            <div className="relative max-h-[68vh] sm:max-h-[72vh] flex items-center justify-center bg-white p-2.5 sm:p-4 rounded-3xl sm:rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.9)] border-4 border-white/90 transform transition-all duration-300">
              <img
                key={selectedPhoto}
                src={CUMPLE_PHOTOS[selectedPhoto].src}
                alt={CUMPLE_PHOTOS[selectedPhoto].alt}
                className="max-h-[62vh] sm:max-h-[66vh] w-auto max-w-full object-contain rounded-2xl animate-in zoom-in-95 duration-200"
              />
            </div>
            ...
            <button
              onClick={handleNext}
              className="absolute right-1 sm:right-2 top-1/2 -translate-y-1/2 w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-amber-400 hover:bg-amber-300 text-slate-950 flex items-center justify-center shadow-2xl transition-all transform hover:scale-110 active:scale-95 cursor-pointer z-30 border-2 border-white/60"
          ...
          <div 
            className="relative z-20 w-full max-w-4xl py-2 flex items-center justify-center gap-2 sm:gap-3 overflow-x-auto px-4"
            onClick={(e) => e.stopPropagation()}
          >
            {CUMPLE_PHOTOS.map((photo, idx) => (
              <button
                ...
                className={`relative shrink-0 w-12 h-12 sm:w-16 sm:h-16 rounded-xl overflow-hidden cursor-pointer transition-all duration-200 border-2 ${
                  selectedPhoto === idx
